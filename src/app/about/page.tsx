export const metadata = {
  title: "About Gordon Amidu | Research Scholar in AI and Social Informatics",
  description:
    "Learn about Gordon Amidu, a research scholar studying artificial intelligence, social informatics, online communities, digital communication, misinformation, and technology-mediated interactions.",
};

export default function About() {
  return (
    <main
      className="
        pt-32
        px-6
        max-w-5xl
        mx-auto
      "
    >
      <h1
        className="
          text-5xl
          font-bold
        "
      >
        About Me
      </h1>

      <div
        className="
          mt-8
          space-y-6
          text-lg
          text-gray-700
          dark:text-gray-300
        "
      >
        <p>
          I'm a research scholar studying the relationship between
          digital technologies, online communities, and social interaction.
          My research explores how emerging technologies shape communication,
          information environments, and human behavior in digital spaces.
        </p>

        <p>
          My research interests center on social informatics, with particular
          emphasis on the role of AI in health and political misinformation,
          AI applications in libraries, and online social interactions.
        </p>

        <p>
          My recent research examines online trolling behaviors on social
          media platforms, with particular attention to platform features,
          contextual collapse, political communication, and gender-related
          online harassment.
        </p>

        <p>
          Through computational and social science approaches, I investigate
          how individuals and communities interact with digital platforms.
          My work combines methods such as content analysis, quantitative
          research, bibliometric analysis, and computational approaches to
          understand online behavior and the broader social implications of
          emerging technologies.
        </p>
      </div>

      <section className="mt-16">
        <h2
          className="
            text-3xl
            font-bold
          "
        >
          Research Interests
        </h2>

        <ul
          className="
            mt-6
            space-y-3
            list-disc
            pl-6
            text-lg
            text-gray-700
            dark:text-gray-300
          "
        >
          <li>
            Artificial intelligence and health misinformation
          </li>

          <li>
            Online trolling, harassment, and toxic behaviors
          </li>

          <li>
            Artificial intelligence applications in libraries
          </li>

          <li>
            Artificial intelligence and political misinformation
          </li>

          <li>
            Social impacts of emerging technologies
          </li>

          <li>
            Library user behavior and information-seeking practices
          </li>
        </ul>
      </section>

      <section className="mt-16">
        <h2
          className="
            text-3xl
            font-bold
          "
        >
          Research Approach
        </h2>

        <p
          className="
            mt-6
            text-lg
            text-gray-700
            dark:text-gray-300
          "
        >
          My research uses interdisciplinary approaches from social
          informatics, information science, and communication studies to
          investigate digital behavior. My work applies quantitative analysis,
          content analysis, bibliometric methods, and computational approaches
          to examine how technology influences social interaction.
        </p>
      </section>
    </main>
  );
}